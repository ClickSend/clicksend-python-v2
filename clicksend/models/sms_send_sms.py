...
class SmsSendSms(BaseModel):
    """
    SmsSendSms
    """ # noqa: E501
    direction: Optional[StrictStr] = Field(default=None, description="It can either be **in** or **out**:  - **in** - You received a message. it has to do with inbound messages.      - **out** \\- You are sending a message. It has to do with outbound messages.", json_schema_extra={"examples": ["in"]})
    ...
    subaccount_id: Optional[StrictInt] = Field(default=None, description="This sub-account of the user. A user can have multiple sub-accounts.", json_schema_extra={"examples": [1]})
    ...
    carrier: Optional[StrictStr] = Field(default=None, description="The phone carrier of the recipient.", json_schema_extra={"examples": ["Telstra"]})
    ...
    __properties: ClassVar[List[str]] = ["direction", "date", "to", "body", "from", "schedule", "message_id", "message_parts", "message_price", "from_email", "list_id", "custom_string", "contact_id", "user_id", "subaccount_id", "is_shared_system_number", "country", "carrier", "status"]
