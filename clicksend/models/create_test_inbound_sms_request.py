...
class CreateTestInboundSmsRequest(BaseModel):
    """
    CreateTestInboundSmsRequest
    """ # noqa: E501
    url: StrictStr = Field(description="The webhook URL where the inbound SMS will be sent to.  <div class=\"info-box\">   <h4><i class=\"fas fa-info-circle\"></i> Note:</h4>   <p>You can create a webhook URL in <em>Pipedream</em> to test receiving the inbound SMS. Follow the instructions <a href=\"https://help.clicksend.com/en/articles/43703-integration-guide-pipedream\" target=\"_blank\">here.</a></p> </div>  ")
    webhook_type: Optional[StrictStr] = Field(default=None, description="Specifies the HTTP method used when the webhook sends a request to the URL. The available options are:  - **post** (Default): Sends the event data using an HTTP POST request.      - **get**: Sends the event data as query parameters in an HTTP GET request.      - **json**: Sends the event data using a custom JSON format (via a POST request).      This parameter is used to determine how the data will be transmitted, depending on your preferred method and how the receiving system is designed to process incoming requests.")
    __properties: ClassVar[List[str]] = ["url", "webhook_type"]
